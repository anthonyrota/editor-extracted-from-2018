...
import { createPointAtEndOfLine, PointType } from "../point";
...
> {
  ...
  return {
    ...
    onNativeBeforeInput(editor, next, event) {
      ...
      // console.log("beforeinput", event.inputType, { targetRange });
    ...
    onInput(editor, next, event) {
      ...
      editor.edit(() => {
        ...
        let newSelectionRange_ = findRange(nativeSelection, document);
        let newSelectionRange = {
          start: newSelectionRange_.anchor,
          end: newSelectionRange_.cursor,
        };
        let rangeOfNode: Range;
        ...
        if (point.type === PointType.InTextNode) {
          ...
          rangeOfNode = createRange(
            point,
            movePointToEndOfInlineNode(document, point)
          );
        } else {
          rangeOfNode = createRange(point, point);
          let p = createPointAtEndOfLine(point.blockNodeIndex);
          newSelectionRange = createRange(p, p);
        }
    ...
    onPaste(editor, next, event) {
      ...
      if (transfer.fragment) {
        insertFragment(
          editor,
          regenerateDocumentKeys(transfer.fragment as any)
        );
